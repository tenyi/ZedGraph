...
namespace ZedGraph
{
	...
	#region Fields
		...
		/// <summary>
		/// 重複 Dispose 防護旗標。C 方案 (H1) 引入。
		/// 防止同一個 Fill 物件被呼叫 Dispose 兩次以上時重複釋放 GDI+ handle。
		/// </summary>
	...
	#region IDisposable
		/// <summary>
		/// C 方案 (H1)：釋放本 Fill 持有的 GDI+ 資源。
		/// 重複呼叫 Dispose 不會拋例外，由 <see cref="_disposed"/> 旗標保護。
		///
		/// 釋放範圍：
		///   - <see cref="_brush"/>    —— Fill 自行建構或外部傳入的 Brush
		///   - <see cref="_gradientBM"/> —— 用於 gradient 的快取 Bitmap（linear gradient cache）
		///
		/// <see cref="_image"/> **不釋放**：它是序列化專用欄位（建構子註解已說明），
		/// 屬於「caller 持有的 Image 物件」，由呼叫端負責 Dispose。
		/// 若 Fill 內部將 _image 做成 TextureBrush 包覆，_brush.Dispose() 時已經
		/// 把 TextureBrush 內部的 image reference 清除；Image 本體仍由 caller 持有。
		///
		/// 沒寫 finalizer 的理由：
		///   Brush / Bitmap / Image 在 .NET Framework 是 SafeHandle 包裹的 managed 資源，
		///   GC 終會回收；寫 finalizer 會引入 finalization queue 成本但實際救不回幾個 handle。
		///   改為「保守 Dispose pattern、不寫 finalizer」是 .NET 實務常見選擇。
		/// </summary>
		public void Dispose()
		{
			if ( _disposed )
	...
	#region Properties
		...
		public Brush Brush
		{
			...
			set
			{
				// C 方案 (H1)：重複指派新 Brush 時，主動 Dispose 舊的，避免 leak。
				// 原 setter 是純欄位指派，使用者改指派多次會讓前一個 Brush 失去引用；
				// 即使 GC + finalizer 終會釋放，及早 Dispose 縮短 GDI+ handle 持有時間。
				if ( _brush != null && _brush != value )
					_brush.Dispose();
				_brush = value;
			}
		}
