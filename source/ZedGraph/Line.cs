...
namespace ZedGraph
{
	...
	#region Rendering Methods
						...
						float scaleFactor, Pen pen, float lastX, float lastY, float tmpX, float tmpY )
		{
			...
			// BUG FIX (H3.1): InterpolatePoint 的最外層 try-catch 原本為空 catch。
			// 此例外吞噬為刻意設計——它是 GDI+ 渲染管線的「最後防線」，用以避免
			// 單一繪製例外中斷整個 chart 的繪製流程（參見上方 1025-1032 行註解：
			// 極端的座標值可能導致 GDI+ 拋例外）。保留「吞例外」契約，但改為
			// 透過 Debug.WriteLine 輸出，方便日後診斷而不改變外部行為。
			catch ( Exception ex )
			{
				System.Diagnostics.Debug.WriteLine(
					$"ZedGraph.Line.InterpolatePoint swallowed exception: {ex.GetType().Name}: {ex.Message}" );
			}
		}
